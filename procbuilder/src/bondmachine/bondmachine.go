...
type Bondmachine struct {
	...
	Links []int // Is the link matrix for Internal_inputs that may be connected only to one Internal_output (they are N!). A value of -1 means the input is unconnected, othervise it point to the Internal_output id

	Shared_objects []Shared_instance      // This is the list of shared elements present in a bondmachine
	Shared_links   []Shared_instance_list // For every processot the list of attached shared objects
...
func (c *Config) ProcbuilderConfig() *procbuilder.Config {
	...
	result.Commented_verilog = c.Commented_verilog
	return result
}
...
func (bmach *Bondmachine) Init() {
	// It contains an idempotent set of operation to ensure bondmachne consistency
		...
		if conf.Dotdetail > 3 {
			...
			if conf.Dotdetail > 4 {
				...
				if disass_text, err := bmach.Domains[dom_id].Disassembler(); err == nil {
					...
					result += "\t\tstruct" + strconv.Itoa(strutti) + " [label=<<TABLE BORDER=\"0\">\n"
					strutti++
