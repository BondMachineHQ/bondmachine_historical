...
func (sm Lfsr8_instance) Get_wires_perproc(bmach *Bondmachine, proc_id int, so_id int, flavor string) string {
	result := ""
...
func (sm Lfsr8_instance) Get_header_perproc(bmach *Bondmachine, proc_id int, so_id int, flavor string) string {
	result := ""
	if soname, ok := bmach.Get_so_name(so_id); ok {
