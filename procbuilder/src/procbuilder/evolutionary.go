...
func (mach *Machine) Mel_init(ep *mel.Evolution_parameters) {

	myarch := new(Arch)

	var eops []string

	if value, ok := ep.Get_value("procbuilder:opcodes"); ok {
	...
	if value, ok := ep.Get_value("procbuilder:rsize"); ok {
	...
	if value, ok := ep.Get_value("procbuilder:r"); ok {
		valuei, _ := strconv.Atoi(value)
	...
	if value, ok := ep.Get_value("procbuilder:l"); ok {
		valuei, _ := strconv.Atoi(value)
	...
	if value, ok := ep.Get_value("procbuilder:n"); ok {
		valuei, _ := strconv.Atoi(value)
	...
	if value, ok := ep.Get_value("procbuilder:m"); ok {
		valuei, _ := strconv.Atoi(value)
	...
	if value, ok := ep.Get_value("procbuilder:o"); ok {
		valuei, _ := strconv.Atoi(value)
...
func (mach *Machine) Mel_copy() mel.Me3li {
	newmach := new(Machine)
...
func Machine_Program_Generate(ep *mel.Evolution_parameters) mel.Me3li {
...
func Machine_Program_Mutate(p mel.Me3li, ep *mel.Evolution_parameters) mel.Me3li {
	var result mel.Me3li
	return result
}

func Machine_Program_Crossover(p mel.Me3li, q mel.Me3li, ep *mel.Evolution_parameters) mel.Me3li {
	var result mel.Me3li
	return result
}
