...
func (op Multf) Op_instruction_verilog_header(conf *Config, arch *Arch, flavor string) string {
	result := ""
...
func (op Multf) Op_instruction_verilog_state_machine(arch *Arch, flavor string) string {
	...
	for i := 0; i < reg_num; i++ {
		...
		for j := 0; j < reg_num; j++ {
			...
			result += "							end\n"
			result += "								$display(\"MULTF " + strings.ToUpper(Get_register_name(i)) + " " + strings.ToUpper(Get_register_name(j)) + "\");\n"
			result += "							end\n"
...
func (Op Multf) Abstract_Assembler(arch *Arch, words []string) ([]UsageNotify, error) {
