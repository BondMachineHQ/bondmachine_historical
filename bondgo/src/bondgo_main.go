...
var show_requirements = flag.Bool("show-requirements", false, "Show bondmachine requirements")

var input_file = flag.String("input-file", "", "Go input file")

var multi_abstract_assembly_input = flag.Bool("multi-abstract-assembly-input", false, "Input from a multi abstract assembly JSON file")
var abstract_assembly_input = flag.Bool("abstract-assembly-input", false, "Input from abstract assembly")
var go_input = flag.Bool("go-input", true, "Input from Go file")
...
var load_bondmachine = flag.String("load-bondmachine", "", "Filename of the bondmachine to load")
var load_machine = flag.String("load-machine", "", "Filename of the machine")

// Saved things
// For optimizing mandatory, optional for the others
var save_machine = flag.String("save-machine", "", "Create a machine JSON file")
var save_bondmachine = flag.String("save-bondmachine", "", "Create a bondmachine JSON file")

// For standard, checking, enforcing
var save_assembly = flag.String("save-assembly", "", "Machine or bondmachine (numbered per domain) assembly output file")
...
func init() {
	...
	if *abstract_assembly_input || *multi_abstract_assembly_input {
		*go_input = false
	}
}
...
func main() {
	...
	if *input_file != "" {
		if *go_input {
			...
			if !bgmain.Is_faulty() {
				...
				case "standard":
					...
					if *mpm {
						machine_to_save = *save_bondmachine
					} else {
						machine_to_save = *save_machine
					}

					if *save_assembly != "" {
						savedsomething = true
						if *mpm || *use_etherbond || *use_udpbond {
							for i, _ := range bgmain.Program {
								if _, err := os.Stat(*save_assembly + "_" + strconv.Itoa(i)); os.IsNotExist(err) {
									f, err := os.Create(*save_assembly + "_" + strconv.Itoa(i))
									check(err)
									defer f.Close()
							...
							if _, err := os.Stat(*save_assembly); os.IsNotExist(err) {
								f, err := os.Create(*save_assembly)
								check(err)
								defer f.Close()
					...
					if *use_etherbond {
						if *save_etherbond_cluster != "" {
							...
							if *etherbond_external != "" {
								...
								ebconfig.Rsize = uint8(*register_size)
							...
							mycluster, peerids, mymachines, myethio, myres, err := bgmain.Create_Etherbond_Cluster(*register_size, external_cluster)
							check(err)
						...
						if *save_udpbond_cluster != "" {
							...
							mycluster, peerids, mymachines, myethio, myres, err := bgmain.Create_Udpbond_Cluster(*register_size, external_cluster)
							check(err)
							//fmt.Println(mycluster, peerids, mymachines, myethio, myres)
						...
						if machine_to_save != "" {
							savedsomething = true
							if *mpm {
								if mymachine, _, err := bgmain.Create_Bondmachine(*register_size, "device_0"); err == nil {
									if _, err := os.Stat(machine_to_save); os.IsNotExist(err) {
										f, err := os.Create(machine_to_save)
										check(err)
										defer f.Close()
								...
								if mymachine, ok := bgmain.Create_Connecting_Processor(*register_size, 0); ok {
									if _, err := os.Stat(machine_to_save); os.IsNotExist(err) {
										f, err := os.Create(machine_to_save)
										check(err)
										defer f.Close()
		...
		} else if *abstract_assembly_input {
			usagedone := make(chan bool)
			...
			bgmain.Abstract_assembler(*register_size, source_asm, usagenotify)
			...
			if *save_bondmachine != "" {
				if mymachine, _, err := bgmain.Create_Bondmachine(*register_size, "device_0"); err == nil {
					if _, err := os.Stat(*save_bondmachine); os.IsNotExist(err) {
		...
		} else if *multi_abstract_assembly_input {
			...
			if *save_bondmachine != "" {
				if mymachine, err := bondgo.MultiAsm2BondMachine(*register_size, aafile); err == nil {
					if _, err := os.Stat(*save_bondmachine); os.IsNotExist(err) {
						f, err := os.Create(*save_bondmachine)
						check(err)
						defer f.Close()
